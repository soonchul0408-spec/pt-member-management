...
function openSession(session) {
  router.push(`/pt/sessions/${session.id}`)
}
</script>
...
<template>
  <div class="sessions-page">
    ...
    <el-card class="page-card" shadow="never">
      <div v-if="sessions.length" class="data-table-wrap"><table class="data-table"><thead><tr><th>수업일</th><th>회원</th><th>운동 초점</th><th>진행한 운동</th><th>컨디션</th><th>상태</th><th></th></tr></thead><tbody><tr v-for="session in sessions" :key="session.id" class="is-clickable" @click="openSession(session)"><td>{{ session.date.replaceAll('-', '.') }}<small class="muted-text">{{ session.startTime || '기록' }} · {{ session.duration ? `${session.duration}분` : '회차 기록' }}</small></td><td><strong>{{ memberName(session.memberId) }}</strong></td><td>{{ session.focus || '-' }}</td><td class="long-text">{{ session.exercises || '-' }}</td><td>{{ session.condition }}</td><td><el-tag :type="session.status === '완료' ? 'success' : session.status === '취소' ? 'info' : 'primary'" size="small" effect="light">{{ session.status }}</el-tag></td><td><el-button v-if="store.canEdit" text type="primary" @click.stop="editingSession = session; dialogOpen = true">수정</el-button><span v-else class="muted-text">상세 보기</span></td></tr></tbody></table></div>
