...
function sessionDetails(session) {
  ...
  if (exerciseText && exerciseText !== genericSessionDescription) details.push({ label: '운동 메모', value: exerciseText })
    ...
    <el-card v-if="notionSessions.length" class="panel-card imported-records-panel" shadow="never">
      ...
      <div class="imported-session-list">
        <article v-for="session in notionSessions" :key="session.id" class="imported-session-item">
          ...
          <video v-if="session.videoUrl" class="imported-session-item__video" controls playsinline :src="session.videoUrl" />
          <p v-if="!sessionDetails(session).length && !session.videoUrl" class="imported-session-item__empty">{{ fallbackSessionDescription(session) }}</p>
...
.imported-session-item__video { display: block; width: min(100%, 420px); max-height: 240px; margin-top: 12px; border-radius: 10px; background: #111827; }
