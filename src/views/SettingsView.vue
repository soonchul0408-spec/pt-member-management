...
<template>
  <div class="settings-page">
    ...
    <div class="settings-grid">
      ...
      <el-card class="panel-card" shadow="never"><div class="section-heading"><div><p class="section-eyebrow">DATA STORAGE</p><h2>데이터 저장 안내</h2></div></div><p class="settings-description">{{ auth.authConfigured ? 'Supabase 로그인 사용자는 서버 데이터와 권한 정책을 적용받습니다. 비로그인 사용자는 샘플 데이터만 조회합니다.' : 'Supabase 환경변수 연결 전에는 샘플 데이터와 localStorage를 사용하는 데모 모드로 동작합니다. 현재는 모든 화면이 읽기 전용입니다.' }}</p><el-button v-if="store.canEdit" type="danger" plain @click="resetDialogOpen = true">샘플 데이터로 초기화</el-button></el-card>
      <el-card class="panel-card" shadow="never"><div class="section-heading"><div><p class="section-eyebrow">ACCESS CONTROL</p><h2>접근 권한</h2></div><el-tag type="info" effect="plain">RLS</el-tag></div><ul class="report-guidelines"><li>비로그인 사용자: 샘플 데이터 조회</li><li>조회자(viewer): 서버 데이터 조회</li><li>편집자(editor): 회원·수업·결제 수정</li><li>관리자(admin): 역할 관리 및 운영</li></ul></el-card>
    </div>
